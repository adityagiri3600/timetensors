...
router.get("/today/:ttid", async (req, res) => {
    const getClassDetails = (classObjects, classItem) => {
        ...
        if (classObjects.length > 0) return classObjects[0];
        return {
    ...
    const getClassesAtDate = (data, date_param) => {
        ...
        return [
            ...classes.filter(
                (classElement) => classElement.Day == date_param.getDay()
            ),
            ...extraClasses,
        ]
            .sort((a, b) => {
    ...
    try {
        ...
        const classesPromises = timetableData.classes.map(async (classItem) => {
            const classData = await classObject.findOne({
...
router.get("/:ttid", async (req, res) => {
    try {
        ...
        const classObjectsPromises = timetableData.classObjects.map(
            async (classItem) => {
                const classData = await classObject.findOne({
                    classid: classItem.classid,
                });
...
router.post("/new", async (req, res) => {
    try {
        ...
        for (let i = 0; i < req.body.classObjects.length; i++) {
            const existingClass = await classObject.findOne({
                classid: req.body.classObjects[i].classid,
            });
            if (!existingClass) {
                const newClassObject = new classObject({
                    ...req.body.classObjects[i],
                    editCode: req.body.editCode,
                });
                await newClassObject.save();
            } else {
                newTimetable.classObjects[i] = existingClass;
            }
        }
...
router.delete("/:ttid", async (req, res) => {
    const { ttid } = req.params;
    ...
    try {
        ...
        await timetable.deleteOne({ ttid });

        res.json("Timetable deleted successfully");
        console.log(`Timetable deleted: ${ttid}`);
    } catch (err) {
        res.status(400).json("Error: " + err.message);
    }
});
